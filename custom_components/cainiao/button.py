"""Button platform for the Cainiao parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Cainiao refresh button from a config entry."""
    async_add_entities([CainiaoRefreshButton(entry)])


class CainiaoRefreshButton(ButtonEntity):
    """Button that forces an immediate poll of all tracked Cainiao parcels."""

    _attr_has_entity_name = True
    _attr_translation_key = "refresh"
    _attr_attribution = ATTRIBUTION

    def __init__(self, entry: CainiaoConfigEntry) -> None:
        self._entry = entry
